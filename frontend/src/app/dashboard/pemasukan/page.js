...
import { apiGet, formatRupiah, formatDateShort } from '@/lib/api';
...
export default function PemasukanPage() {
  const {
    ...
    toast, createItem, updateItem, deleteItem
  } = useCrud('incomes', { label: 'Pemasukan', perPage: 15 });
  ...
  const [form, setForm] = useState({ category_id: '', donor_id: '', amount: '', date: '', description: '', receipt_number: '' });

  useEffect(() => {
    apiGet('income-categories').then(r => setCategories(r.data || [])).catch(() => {});
    apiGet('donors', { per_page: 100 }).then(r => setDonors(r.data || [])).catch(() => {});
  }, []);

  function openAdd() {
    setEditData(null);
    setForm({ category_id: '', donor_id: '', amount: '', date: new Date().toISOString().split('T')[0], description: '', receipt_number: '' });
    setModalOpen(true);
  }
  function openEdit(item) {
    setEditData(item);
    setForm({ category_id: item.category_id, donor_id: item.donor_id || '', amount: item.amount, date: item.date, description: item.description || '', receipt_number: item.receipt_number || '' });
    setModalOpen(true);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const payload = { ...form, amount: parseFloat(form.amount), donor_id: form.donor_id || null };
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      <div className="relative overflow-hidden rounded-3xl shadow-lg shadow-emerald-900/5 animate-fade-in bg-white border border-gray-100">
        ...
        <div className="relative z-10 p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-black text-gray-800 tracking-tight">Catat Pemasukan</h1>
              <p className="text-sm font-medium text-gray-500 mt-1">Kelola transaksi pemasukan dana (ZIS)</p>
            </div>
          </div>
      ...
      <div className="glass-card rounded-3xl p-4 md:p-5 animate-fade-in bg-white/90 border border-gray-100 shadow-sm" style={{animationDelay:'100ms'}}>
        <div className="flex flex-col md:flex-row gap-4 w-full">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari transaksi..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-gray-50/50 border border-gray-200 rounded-2xl text-sm font-medium focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all placeholder-gray-400" />
          </div>
          <div className="flex flex-col sm:flex-row gap-4 shrink-0">
            <select value={filters.category_id || ''} onChange={e => setFilters({...filters, category_id: e.target.value})} className="w-full sm:w-48 px-4 py-3 bg-gray-50/50 border border-gray-200 rounded-2xl text-sm font-medium focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all appearance-none">
              <option value="">Semua Kategori</option>
              {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <div className="flex items-center gap-2">
              <input type="date" value={filters.date_from || ''} onChange={e => setFilters({...filters, date_from: e.target.value})} className="w-full sm:w-40 px-4 py-3 bg-gray-50/50 border border-gray-200 rounded-2xl text-sm font-medium focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all" />
              <span className="text-gray-400 font-bold">—</span>
              <input type="date" value={filters.date_to || ''} onChange={e => setFilters({...filters, date_to: e.target.value})} className="w-full sm:w-40 px-4 py-3 bg-gray-50/50 border border-gray-200 rounded-2xl text-sm font-medium focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-sm">
            <thead><tr className="bg-gradient-to-r from-emerald-50/50 to-green-50/50 border-b border-gray-100">
              ...
              <th className="text-left px-6 py-4 font-bold text-gray-700 tracking-wide uppercase text-xs hidden md:table-cell">Donatur (Muzakki)</th>
              ...
              <th className="text-right px-6 py-4 font-bold text-gray-700 tracking-wide uppercase text-xs">Jumlah</th>
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              items.length === 0 ? <tr><td colSpan={5} className="text-center py-16">
                ...
                <p className="text-gray-400 text-sm mt-1">Ganti filter atau tambahkan pemasukan baru.</p>
              </td></tr> :
              items.map(d => (
                <tr key={d.id} className="hover:bg-emerald-50/30 transition-colors group">
                  <td className="px-6 py-4">
                    ...
                    <span className="inline-block px-2.5 py-1 rounded-lg text-[11px] font-bold bg-emerald-100/50 text-emerald-700 border border-emerald-100">{d.category_name}</span>
                  </td>
                  <td className="px-6 py-4 hidden md:table-cell">
                    {d.donor_name ? (
                      <p className="font-bold text-gray-700 group-hover:text-emerald-700 transition-colors">{d.donor_name}</p>
                    ) : (
                      <p className="text-gray-400 font-medium italic">Hamba Allah / Umum</p>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <span className="inline-block px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 font-black text-sm border border-emerald-100/50 shadow-sm">{formatRupiah(d.amount)}</span>
                  </td>
                  <td className="px-6 py-4">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editData ? 'Edit Data Pemasukan' : 'Form Pemasukan Dana (ZIS)'}>
        <div className="space-y-5 p-2">
          <FormField label="Kategori ZIS" required><Select value={form.category_id} onChange={e => setForm({...form, category_id: e.target.value})}><option value="">Pilih kategori dana</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</Select></FormField>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <FormField label="Jumlah Uang (Rp)" required><Input type="number" min="0" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} placeholder="Contoh: 500000" /></FormField>
            <FormField label="Tanggal Transaksi" required><Input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})} /></FormField>
          </div>
          <FormField label="Nama Donatur (Muzakki)"><Select value={form.donor_id} onChange={e => setForm({...form, donor_id: e.target.value})}><option value="">-- Hamba Allah / Umum --</option>{donors.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</Select></FormField>
